...
const Sidebar = ({ isOpen, isCollapsed = false, onClose, onToggle }: SidebarProps) => {
  ...
  const navigation = [
    ...
    {
      name: user?.role === 'user' ? 'My Labs' : 'Assessments',
      href: '/assessments',
      icon: '📝',
      show: isAuthenticated,
      description: user?.role === 'user' ? 'Active practice labs' : 'Tests & Quizzes'
    },
    {
      name: 'Problems',
      href: '/problems',
  ...
  const adminModules = [
    {
      name: 'My Department',
      href: '/admin/my-department',
      icon: '🏛️',
      show: user?.role === 'admin',
      description: 'Department overview & analytics'
    },
    {
      name: 'Admin Console',
      href: '/admin',
  ...
  return (
    <>
      ...
      <div className={cn(
        'fixed inset-y-0 left-0 z-[70] bg-white border-r border-gray-200 transform transition-all duration-300 ease-in-out',
        ...
        <div className="flex flex-col h-full">
          ...
          <div className={cn(
            ...
            <Link to="/" className="flex items-center space-x-3">
              ...
              {!isCollapsed && (
                <div>
                  <div className="font-bold text-gray-900">{config.app.name}</div>
          ...
          {isAuthenticated && user && (
            <div className={cn(
              ...
              >
                <div className="w-10 h-10 bg-black rounded-lg flex items-center justify-center text-white font-semibold flex-shrink-0">
