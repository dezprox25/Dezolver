...
interface User {
  ...
  email: string
  full_name?: string
  role: 'user' | 'admin' | 'hr_manager' | 'manager' | 'super_admin' | 'platform_admin'
...
export const isManager = (user: User | null): boolean => {
  return user?.tier === 'manager' || isPlatformAdmin(user)
}
...
export const canAccessBankDetails = (user: User | null): boolean => {
  return hasOrganization(user) && (isManager(user) || isHRManager(user) || isAdmin(user))
}
